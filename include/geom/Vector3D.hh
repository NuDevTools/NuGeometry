...
class Vector3D {
  public:
    ...
    bool operator==(const Vector3D &other) const { return m_vec == other.m_vec; }
    ...
    Vector3D &operator/=(double scale) { return *this *= 1.0 / scale; }
    ...
    Vector3D &operator-=(const Vector3D &other) { return *this += -other; }
    Vector3D operator*(double scale) const { return Vector3D{*this} *= scale; }
    double operator*(const Vector3D &other) const { return Dot(other); }
    Vector3D operator+(const Vector3D &other) const { return Vector3D{*this} += other; }
    Vector3D operator-(const Vector3D &other) const { return Vector3D{*this} -= other; }
    Vector3D operator-() const { return {-m_vec[0], -m_vec[1], -m_vec[2]}; }

    template <typename OStream> friend OStream &operator<<(OStream &os, const Vector3D &vec) {
