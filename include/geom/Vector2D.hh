...
class Vector2D {
  public:
    ...
    bool operator==(const Vector2D &other) const { return m_vec == other.m_vec; }
    ...
    Vector2D &operator/=(double scale) { return *this *= 1.0 / scale; }
    ...
    Vector2D &operator-=(const Vector2D &other) { return *this += -other; }
    Vector2D operator*(double scale) const { return Vector2D{*this} *= scale; }
    Vector2D operator+(const Vector2D &other) const { return Vector2D{*this} += other; }
    Vector2D operator-(const Vector2D &other) const { return Vector2D{*this} -= other; }
    Vector2D operator-() const { return {-m_vec[0], -m_vec[1]}; }
