...
class DetectorSim {
  public:
    ...
    std::pair<Vector3D, NuGeom::Element> GetInteraction(Vector3D position, Vector3D direction,
                                                        double energy) {
        ...
        size_t idx = 0;
        double total_prob = std::accumulate(
            segments.begin(), segments.end(), 0.0, [mfp, &probs, &idx](double a, LineSegment &b) {
                std::cout << b.GetMaterial().Name() << std::endl;
                auto mean_free_path = mfp.at(b.GetMaterial());
                probs[idx] = 1 - exp(-b.Length() / mean_free_path);
                std::cout << idx << " " << probs[idx] << " " << b.Length() << " " << mean_free_path
                          << std::endl;
                return a + probs[idx++];
            });
  ...
  private:
    ...
    double CalcCrossSection(const NuGeom::Material &mat, double nu_energy) {
        ...
        for(size_t i = 0; i < mat.NElements(); ++i) {
            m_xsec[mat].push_back(mat.MassFractions()[i] *
                                  CalcCrossSection(mat.Elements()[i], nu_energy));
            std::cout << mat.Elements()[i].Name() << " " << mat.MassFractions()[i] << " "
                      << CalcCrossSection(mat.Elements()[i], nu_energy) << std::endl;
