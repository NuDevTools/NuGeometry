...
class Vector4D {
  public:
    ...
    bool operator==(const Vector4D &other) const { return m_vec == other.m_vec; }
    ...
    Vector4D &operator/=(double scale) { return *this *= 1.0 / scale; }
    ...
    Vector4D &operator-=(const Vector4D &other) { return *this += -other; }
    Vector4D operator*(double scale) const { return Vector4D{*this} *= scale; }
    double operator*(const Vector4D &other) const { return Dot(other); }
    Vector4D operator+(const Vector4D &other) const { return Vector4D{*this} += other; }
    Vector4D operator-(const Vector4D &other) const { return Vector4D{*this} -= other; }
    Vector4D operator-() const { return {-m_vec[0], -m_vec[1], -m_vec[2], -m_vec[3]}; }

    template <typename OStream> friend OStream &operator<<(OStream &os, const Vector4D &vec) {
