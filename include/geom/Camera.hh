...
class Camera {
  public:
    ...
    void MoveForward(double distance) { m_pos += m_forward * distance; }

    void MoveRight(double distance) { m_pos += m_right * distance; }

    void MoveUp(double distance) { m_pos += m_up * distance; }
    ...
    void PreComputeRays() {
        ...
        for(uint32_t y = 0; y < imgHeight; ++y) {
            for(uint32_t x = 0; x < imgWidth; ++x) { m_rays[y * imgWidth + x] = MakeRay(x, y); }
        }
    }
