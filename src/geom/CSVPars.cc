...
std::vector<NuGeom::NuRay> ParseCSVFlux(const std::string &filename) {
    ...
    if(!file.is_open()) {
        NuGeom::Log().error("Could not open file: {}", filename);
    ...
    while(std::getline(file, line)) {
        ...
        if(tokens.size() < 9) {
            NuGeom::Log().warn("Skipping malformed line: {}", line);
            continue;
        }
        ...
        try {
            ...
            if(Enu < 0) {
                NuGeom::Log().warn("Negative energy: {}", line);
                continue;
            }
            ...
            NuGeom::Log().warn("Skipping invalid line: {} ({})", line, e.what());
        }
    }
...
int main(int argc, char *argv[]) {
    if(argc < 2) {
        NuGeom::Log().error("Usage: {} <csv_file>", argv[0]);
        return -1;
    }
    ...
    try {
        ...
        outfile.close();
        NuGeom::Log().info("Parsed {} neutrino rays from CSV", nu_rays.size());
        double total_weight = ComputeTotalWeight(nu_rays);
        NuGeom::Log().info("Total weight of parsed neutrino rays: {}", total_weight);
        double weighted_energy = ComputeWeightedEnergy(nu_rays);
        weighted_energy /= total_weight;
        NuGeom::Log().info("Weighted energy of parsed neutrino rays: {}", weighted_energy);

    } catch(const std::exception &e) {
        NuGeom::Log().error("Error parsing CSV: {}", e.what());
        return -1;
    }
