...
Vector3D CalcNormal(NuGeom::World world, const Vector3D &pos) {
    ...
    return Vector3D(shape->SignedDistance(pos + eps * xAxis) - c,
                    ...
                    shape->SignedDistance(pos + eps * zAxis) - c)
        .Unit();
}

void PixelColor(const NuGeom::World &world, const NuGeom::Camera &camera, size_t i, size_t j,
                Vector3D &color, Vector3D & /*cost*/) {
    // Get ray
    ...
    if(!hit || idx == 0) {
        Vector3D offset(ray.Direction().Y() * 0.4, ray.Direction().Y() * 0.4,
                        ray.Direction().Y() * 0.4);
        ...
        if(idx == 1)
            objectSurfaceColor = Vector3D(0.4, 0.5, 0.1);
        else if(idx == 2)
            objectSurfaceColor = Vector3D(0.1, 0.2, 0.8);
        else if(idx == 3)
            objectSurfaceColor = Vector3D(0.8, 0.2, 0.8);
        else if(idx == 4)
            objectSurfaceColor = Vector3D(0.1, 0.8, 0.1);
    ...
    color = Vector3D(pow(color.X(), 0.4545), pow(color.Y(), 0.4545), pow(color.Z(), 0.4545));
...
void render(const NuGeom::World &world, const NuGeom::Camera &camera, std::vector<Vector3D> &pixels,
            std::vector<Vector3D> pixelsCost) {
...
void line(int x0, int y0, int x1, int y1, std::vector<Vector3D> &pixels) {
    ...
    if(steep) {
        for(int x = x0; x <= x1; x++) {
            pixels[static_cast<size_t>(y) * NuGeom::Camera::Height() + static_cast<size_t>(x)] =
                Vector3D(1, 0, 0);
            error2 += derror2;
            if(error2 > dx) {
                y += (y1 > y0 ? 1 : -1);
        ...
        for(int x = x0; x <= x1; x++) {
            pixels[static_cast<size_t>(x) * NuGeom::Camera::Height() + static_cast<size_t>(y)] =
                Vector3D(1, 0, 0);
            error2 += derror2;
            if(error2 > dx) {
                y += (y1 > y0 ? 1 : -1);
...
void renderRay(const NuGeom::Ray &ray, double t, std::vector<Vector3D> &pixels) {
    ...
    int y0 =
        static_cast<int>((start.Y() + 1) * static_cast<double>(NuGeom::Camera::Height()) / 2.0);
...
int main(int argc, char *argv[]) {
    ...
    for(size_t i = 0; i < nrays; ++i) {
        NuGeom::Ray ray =
            neutrino.MakeRay(NuGeom::Camera::Width() / 2, NuGeom::Camera::Height() / 2);
    ...
    for(const auto &line : lines) { std::cout << line.Length() << "\n"; }
